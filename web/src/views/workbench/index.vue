...
const currentMode = ref("squareTool"); // 当前模式
...
const switchMode = (mode) => {
  ...
  if (mode === "selectBox") isDrawing = false;
  if (mode === "squareTool") isDrawing = true;
};
...
const selectedRect = computed(() => {
  if (
    ...
    boxes.value[selectedRectIndex.value]
  ) {
    return boxes.value[selectedRectIndex.value];
  }
  return null;
});
...
const handleMouseDown = (e) => {
  ...
  if (e.target === stage || currentMode.value !== "squareTool") return;
  ...
  selectedRectIndex.value = boxes.value.length - 1; // 记录当前选中的矩形
    ...
    <div class="flex justify-between py-4 px-10">
      <div class="flex flex-col gap-4 mt-8">
        ...
        <n-button
          strong
          secondary
          :type="currentMode === 'squareTool' ? 'primary' : 'tertiary'"
          @click="switchMode('squareTool')"
        >
          矩形工具
          <template #icon>
